(($) => {
	$.fn.seaFlow = function(options) {
		var settings	= $.extend({}, $.fn.seaFlow.defaults, options),
			el			= this,
			isPaused	= false,
			isLoaded	= false,
			app;
		...
		var Water = function( context ) {
			...
			function init() {
				settings.waveComeUp = this.start.bind( this );
			}
		...
		var Wave = function( originalX, originalY, context, offset ) {
			...
			function init() {
				...
				var anchor;
				var current = 0;
				var start = - settings.waveAmplitude;
				var target = settings.waveAmplitude;
				var delta = offset;
				var step = 0.4;

				times( window.innerWidth / settings.waveLength, function() {
			...
			function update() {
				...
				y += ( targetY - y ) / settings.waveRiseSpeed;
		...
		var Anchor = function( x, y, start, target, delta ) {
			...
			this.update = function( mouseDirection, currentY ) {
				...
				var vector = {
					x: mouseDirection.x * factor * settings.waveMouse,
					y: mouseDirection.y * factor * settings.waveMouse
				};
			...
			function getMultiplier() {
				...
				var radius = settings.waveRadius;
		...
		var Motion = function( start, target, delta ) {
			...
			this.onAmpChange = function() {
				min = - settings.waveAmplitude;
				max = settings.waveAmplitude;
				half = min + ( max - min ) / 2;
		...
		var Spring = function() {
			...
			this.update = function() {
				vx = targetX - this.x;
				vy = targetY - this.y;
				px = px * settings.waveElasticity + vx * settings.waveStrength;
				py = py * settings.waveElasticity + vy * settings.waveStrength;
				this.x += px;
				this.y += py;
			};

			this.shoot = function( vector ) {
				targetX = clamp( vector.x, -settings.waveMax, settings.waveMax );
				targetY = clamp( vector.y, -settings.waveMax, settings.waveMax );
